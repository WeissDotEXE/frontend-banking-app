...
const FriendModal: FC<FriendModalProps> = (props: FriendModalProps) => {
    ...
    const deleteFriendHandler = async () => {
        try {
            const response = await axios.delete(`${process.env.REACT_APP_BASE_URL}/friends/deleteFriend/${_id}`);
    ...
    return (
        <Modal onClose={onClose} className={rootCls}>
            ...
            {showRemove ? <div className={friendContentCls}>
                    <RegularSubtitle>Are you sure you want to remove <strong>{fullName}</strong> from your friend
                        list?</RegularSubtitle>
                    <div className={buttonsCls}>
                        <Button type={"button"} onClick={deleteFriendHandler}>Yes</Button>
                        <Button type={"button"} onClick={closeRemoveContent}>No</Button>
                    </div>
                </div> :
